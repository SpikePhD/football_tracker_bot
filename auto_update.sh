...
if [ "$LOCAL_SHA" == "$REMOTE_SHA" ]; then
    ...
    # `git reset --hard origin/$GIT_BRANCH` is an alternative if you always want to match remote.
    # For now, a simple pull. Add --ff-only if you only want fast-forwards.
    ...
    # This checks if requirements.txt was among the files changed between the old HEAD and new HEAD
    ...
    # --no-pager prevents interactive mode if status output is long
