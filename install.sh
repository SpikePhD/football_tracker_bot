...
if [ "$PY_MAJOR" -eq 3 ] && [ "$PY_MINOR" -eq 12 ]; then
    ok "Python $PY_VERSION (3.12 required)"
else
    err "Python $PY_VERSION found but Python 3.12 is required."
    MISSING=1
fi
...
prompt_with_default "Dashboard service name" "marco_van_botten_dashboard" DASHBOARD_SERVICE_NAME
prompt_with_default "Managed update service name" "marco_van_botten_update" UPDATE_SERVICE_NAME
prompt_with_default "Dashboard listen port" "8765" DASHBOARD_PORT
...
if [ -f "$BOT_DIR/.env.deploy" ]; then
    ok ".env.deploy already exists — not overwritten"
else
    cat > "$BOT_DIR/.env.deploy" <<EOF
# .env.deploy — deployment configuration (generated by install.sh)
...
EOF
    ok ".env.deploy created (SERVICE_NAME=$SERVICE_NAME, GIT_BRANCH=$GIT_BRANCH)"
fi
...
if [ -f "$BOT_DIR/bot_memory/tennis_state.json" ]; then
    ok "bot_memory/tennis_state.json already exists - keeping existing state"
else
    echo '{"version": 2, "matches": {}}' > "$BOT_DIR/bot_memory/tennis_state.json"
...
bash "$BOT_DIR/install_dashboard.sh"
ok "$DASHBOARD_SERVICE_NAME installed and started"
